...
x1=[-1.5,4]#第一个决策变量范围
x2=[-3,4]#第二个决策变量范围
b1=[1,1]#第一个决策变量边界，1表示包含范围的边界，0表时不包含
b2=[1,1] #第二个决策变量边界，1表示包含范围的边界，0表时不包含
#生成自变量的范围矩阵，使得第一行为所有决策变量的下界，第二行为上界
ranges=np.vstack([x1,x2]).T
#生成自变量的边界矩阵
borders=np.vstack([b1,b2]).T
varTypes=np.array([0,0])#决策变量的类型，0表时连续，1表示离散
"""===============染色体编码设置=============="""
Encoding='BG' #BG表示采取二进制/格雷编码
codes=[1,1] #决策变量的编码方式，两个1表示变量均采用格雷编码
...
NIND=20#种群个体数目
MAXGEN=100#最大遗传代数
maxormins=np.array([1])#表示目标函数是最小化，元素为-1则表示对应的目标函数最大化
...
for gen in range(MAXGEN):
	FitnV=ea.ranking(maxormins * ObjV) #根据函数大小分配适应度值
